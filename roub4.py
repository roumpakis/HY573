...
def animate_spline(history, L1, L2, L3, target, initial_angles, annotate=True, start_frame=0):
    """
    ...
    # Text box to display the initial angles
    text_box = ax.text(0.1, 0.95, f'Initial Angles: {initial_angles}', transform=ax.transAxes,
    ...
    def update(frame):
        ...
        # Update the initial angles in the text box
        text_box.set_text(f'Initial Angles: {initial_angles}')
...
# Step 2: Create and save animations for all random initializations
all_animations = []
start_frame = 0

for initial_angles in random_angles:
    # Optimize for the current random initialization
    history = wrap_and_optimize(
        L1, L2, L3, x_t, y_t, t=0.1, x_init=initial_angles
    )
    
    # Create animation for this random start
    ani = animate_spline(history, L1, L2, L3, target=(x_t, y_t), initial_angles=initial_angles, start_frame=start_frame)

    # Update start frame for the next animation
    start_frame += len(history)

    all_animations.append(ani)

# Combine all animations into a single animation
combined_ani = animation.FuncAnimation(
    plt.gcf(),
    lambda frame: all_animations[frame % len(all_animations)].event_source,
    #frames=len(all_animations) * max(len(history) for history in all_animations),
...
combined_ani.save('./roub4.gif', writer='pillow', fps=10)
